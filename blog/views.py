...
def category(request, category):
    category_posts = Post.objects.filter(category=category).order_by('-time')
...
def categories(request):
    all_categories = Post.objects.values('category').distinct().order_by('category')
...
def search(request):
    ...
    for word in query_list:
        results = results | Post.objects.filter(Q(title__contains=word) | Q(content__contains=word)).order_by('-time')
